...
void displayGalaxy(const Galaxy &galaxy)
{
    ...
    SDL_Window *window = SDL_CreateWindow("Galaxy Map",
    ...
    while (!quit)
    {
        ...
        while (SDL_PollEvent(&e) != 0)
        {
            ...
            else if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT)
            {
                ...
                for (const System &system : systems)
                {
                    ...
                    {
                        selectedSystem = const_cast<System *>(&system); // Храним выбранную систему
                        break;
                    }
                }
            }
        ...
        for (const Line &line : connections)
        {
            SDL_RenderDrawLine(renderer,
                               line.start.x * CELL_SIZE,
                               line.start.y * CELL_SIZE,
                               line.end.x * CELL_SIZE,
        ...
        for (const System &system : systems)
        {
            ...
            if (isHovered)
            {
                ...
                rect.w = HOVER_SIZE;
                rect.h = HOVER_SIZE;
                SDL_SetRenderDrawColor(renderer, 243, 139, 168, 255); // Красный - парит
            }
            else if (&system == selectedSystem)
            {
                SDL_SetRenderDrawColor(renderer, 137, 180, 250, 255); // Синий - выбранная
            }
            else if (system.hasHabitables())
            {
                SDL_SetRenderDrawColor(renderer, 166, 227, 161, 255); // Зеленый - обитаемая
            }
            else
            {
                SDL_SetRenderDrawColor(renderer, 249, 226, 175, 255); // Желтый - обычнвя
            }
        ...
        if (selectedSystem != nullptr)
        {
            ...
            const int INFO_BOX_HEIGHT = 400;

            // Фон коробки
            SDL_Rect infoBox = {
                SCREEN_WIDTH - INFO_BOX_WIDTH - 10,
                10,
                INFO_BOX_WIDTH,
                INFO_BOX_HEIGHT};
            ...
            SDL_Surface *surface;
            SDL_Texture *texture;
            TTF_Font *font = TTF_OpenFont("/home/nahmaida/Ad-Astra/res/bytebounce_medium.ttf", 24);
            if (font == nullptr)
            {
            ...
            {
                // Выводим имя системы
                surface = TTF_RenderText_Solid(font, selectedSystem->getName().c_str(), textColor);
                texture = SDL_CreateTextureFromSurface(renderer, surface);
                SDL_Rect textRect = {infoBox.x + 10, infoBox.y + 10, surface->w, surface->h};
                SDL_RenderCopy(renderer, texture, nullptr, &textRect);
                SDL_FreeSurface(surface);
                SDL_DestroyTexture(texture);

                // Выводим планеты
                const auto &planets = selectedSystem->getPlanets();
                int yOffset = 30;
                for (const Planet &planet : planets)
                {
                    surface = TTF_RenderText_Solid(font, ("-" + planet.getName()).c_str(), textColor);
                    texture = SDL_CreateTextureFromSurface(renderer, surface);
                    textRect = {infoBox.x + 20, infoBox.y + yOffset, surface->w, surface->h};
                    SDL_RenderCopy(renderer, texture, nullptr, &textRect);
                    SDL_FreeSurface(surface);
                    SDL_DestroyTexture(texture);
                    yOffset += 20;
